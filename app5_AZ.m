...
K_AvPh_B = 1/abs(evalfr(FTBO_AZ, j*wg_B_AZ));


[Gm_AZ Pm_AZ wgm wpm] = margin(K_AvPh_B*FTBO_AZ);

phi_AvPh_AZ_B = (des_PM_B+mrg_PM_AZ) - Pm_AZ;
alpha_AZ_B = ( 1-sind(phi_AvPh_AZ_B) )/( 1+sind(phi_AvPh_AZ_B) );
T_AZ_B = 1/(wg_B_AZ*sqrt(alpha_AZ_B));
...
T_AZ_Re_B = 10/wg_B_AZ;

z = -1/T_AZ_Re_B;
% z = -0.68
p = -1/(K_RePh_B*T_AZ_Re_B);

Kr_AZ_B = 1/abs((j*wg_B_AZ - z)/(j*wg_B_AZ-p));
% Kr_AZ_B = 0.95;
